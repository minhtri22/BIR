...
| REQ-013 | SRS Section 7 UC-04; SRS Section 11 Analysis jobs; BA BP-03; BA BR-002 | Analysis jobs are asynchronous. | Retry must not create uncontrolled duplicates. | Analysis Orchestrator, Worker, Job Store; analysis job API | Phase 3 | Planned | Revised design contract defines `request_fingerprint`, per-attempt `attempt_no`, one active job per project, retry lineage, and worker integration tests |
...
| TC-02 | Duplicate analysis by artifact hash + server-owned analyzer version/configuration does not create uncontrolled duplicate candidates. | REQ-013 | Phase 3 | Planned | Request fingerprint, attempt retry, active-job, and worker/orchestrator idempotency tests |
...
Additional Phase 3 design test cases:
...
- Static extraction contract is revised in `docs/implementation_contract/phase3_static_extraction_contract.md`.
